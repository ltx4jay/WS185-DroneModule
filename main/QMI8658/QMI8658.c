...
void QMI8658_Init(void)
{
    ...
    I2C_Read(Device_addr, QMI8658_WHO_AM_I, &whoAmI, sizeof(whoAmI));
    I2C_Read(Device_addr, QMI8658_REVISION_ID, &revision, sizeof(revision));
    ESP_LOGI("QMI8658", "Device ID: %X  Rev: %X\n", whoAmI, revision);
...
QMI8658_Loop(void)
{
    ...
    // Average the data over a sampling window and replace immediate sample with that
    static IMUdata      sampleWin[2]  = {};
    static const size_t cSampleWinSize = sizeof(sampleWin) / sizeof(sampleWin[0]);
    static size_t       nextSample     = 0;

    sampleWin[nextSample] = Accel;
    nextSample            = (nextSample + 1) % cSampleWinSize;

    Accel.x = 0;
    Accel.y = 0;
    Accel.z = 0;
    for (int i = 0; i < cSampleWinSize; i++) {
        Accel.x += sampleWin[i].x;
        Accel.y += sampleWin[i].y;
        Accel.z += sampleWin[i].z;
    }
    Accel.x /= cSampleWinSize;
    Accel.y /= cSampleWinSize;
    Accel.z /= cSampleWinSize;
}
